                        ...
                        <div class="cartWrap">
                            <div class="controls">
                                ...
                                <a
                                    href="javascript:"
                                    class="plus"
                                    @click="changeGoodsNum('increment')"
                                    >+</a
                                >
                                <a
                                    href="javascript:"
                                    class="mins"
                                    @click="changeGoodsNum('decrement')"
                                    >-</a
                                >
                            </div>
                            <div class="add">
...
export default {
    ...
    methods: {
        ...
        async addCart() {
            ...
            if (code === 200) {
                ...
                const { price, spuSaleAttrList } = this.info;
                const { skuName, skuDefaultImg } = this.info.skuInfo;
                const { goodsNum } = this;

                const attrArr = [];
                spuSaleAttrList.forEach((s1) => {
                    let result = s1.spuSaleAttrValueList.find(
                        (s2) => s2.isChecked === "1"
                    );
                    attrArr.push(
                        result.saleAttrName + "：" + result.saleAttrValueName
                    );
                });
                const buyInfo = {
                    skuName,
    ...
    mounted() {
        // 组件一挂载就发请求获取详情数据
        this.getGoodsInfo('/addcart_success');
    },
};
</script>
