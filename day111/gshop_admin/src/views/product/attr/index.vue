                ...
                <el-table v-loading="loading" :data="attrList" stripe border style="width: 100%">
                    ...
                    <el-table-column prop="attrValueList" label="属性值列表">
                        <template #default="{ row, $index }">
                            <el-tag type="success" v-for="(item, index) in row.attrValueList"
                                :key="index">{{ item.valueName }}</el-tag>
                        </template>
                    </el-table-column>
...
import type { AttrInfoListModel,AttrInfoModel } from '@/api/product/model/attrModel'
