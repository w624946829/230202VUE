<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <ValidationProvider name="手机号" v-slot="{errors}" :rules="">
          <input v-model="phone" type="text" placeholder="请输入你的手机号" />
          <span class="error-msg">{{errors[0]}}错误提示信息</span>
        </ValidationProvider>
      </div>
      <div class="content">
        <label>验证码:</label>
...
import {reqGetCode,reqRegister} from '@/api'
  export default {
    ...
    methods:{
      async getCode(){
        let {code,message,data} = await reqGetCode(this.phone)
        if(code === 200){
      ...
      async register(){
        ...
        let {code:resCode,message,data} = await reqRegister({phone,password,code})
